...
class NrwBezirksvertretungAllocator {
    ...
    calculate(partiesData, initialTotalSeats) {
        ...
        const excludedVotes = excludedParties.reduce((sum, excludedParty) => {
            const party = parties.find((entry) => entry.id === excludedParty.id);
            return sum + (party ? party.votes : 0);
        }, 0);
        const adjustedTotalVotes = totalVotes - excludedVotes;

        protocolEntries.push({
            type: 'threshold-summary',
            legalReference: '\u00a7 46a Abs. 6',
        ...
        const getFivePercentPartiesWithoutSeat = () => partiesAtFivePercent
            .filter((party) => getSeatCountForParty(party.id) === 0)
            .map((party) => {
                const originalParty = parties.find((entry) => entry.id === party.id);
                return {
                    id: party.id,
                    abbreviation: party.abbreviation,
                    voteSharePercent: originalParty ? originalParty.voteSharePercent : 0
                };
            });
        const initiallyAffectedParties = getFivePercentPartiesWithoutSeat();
        ...
        if (currentSeats > initialTotalSeats) {
            protocolEntries.push({
                ...
                to: currentSeats,
                step: 2,
                iterations: (currentSeats - initialTotalSeats) / 2,
                legalReference: '\u00a7 46a Abs. 7',
                reason: 'Eine Liste ab 5% der Gesamtstimmenzahl haette sonst keinen Sitz erhalten.',
                affectedParties: initiallyAffectedParties
            });
        }
